...
public class AddSaleItemHandler(ISaleRepository repo) : IRequestHandler<AddSaleItemCommand, Result>
{
    public async Task<Result> Handle(AddSaleItemCommand cmd, CancellationToken ct)
    {
        ...
        if (sale is null) return Result.NotFound<Sale>(cmd.SaleId);
...
public class CompleteSaleHandler(ISaleRepository repo, ISaleEventPublisher publisher)
    : IRequestHandler<CompleteSaleCommand, Result<CompleteSaleResponse>>
{
    public async Task<Result<CompleteSaleResponse>> Handle(CompleteSaleCommand cmd, CancellationToken ct)
    {
        var sale = await repo.GetByIdAsync(cmd.SaleId, ct);
        if (sale is null)                    return Result.NotFound<CompleteSaleResponse, Sale>(cmd.SaleId);
        ...
        await repo.SaveChangesAsync(ct);
        await publisher.PublishSaleCompletedAsync(sale, ct);

        return Result.Success(new CompleteSaleResponse(
            sale.Id, sale.ReceiptNumber!, sale.TotalAmount, sale.ChangeAmount));
    }
}
