...
public record CreateOrganizationTypeCommand(string Code, string NameUz, string NameRu, string? NameEn = null, string? NameUzCyrl = null, string? NameKk = null, string? Icon = null)
    ...
    : IRequestHandler<CreateOrganizationTypeCommand, Result<Guid>>
{
    ...
    {
        var orgType = OrganizationType.Create(Guid.NewGuid(), cmd.Code, cmd.NameUz, cmd.NameRu, cmd.NameEn, cmd.NameUzCyrl, cmd.NameKk, cmd.Icon);
        await repo.AddAsync(orgType, ct);
        return Result.Success(orgType.Id);
    }
}

public record UpdateOrganizationTypeCommand(Guid Id, string NameUz, string NameRu, string? NameEn = null, string? NameUzCyrl = null, string? NameKk = null, string? Icon = null)
    : IRequest<Result>;
    ...
    : IRequestHandler<UpdateOrganizationTypeCommand, Result>
{
    public async Task<Result> Handle(UpdateOrganizationTypeCommand cmd, CancellationToken ct)
    {
        ...
        orgType.Update(cmd.NameUz, cmd.NameRu, cmd.NameEn, cmd.NameUzCyrl, cmd.NameKk, cmd.Icon);
